...
<html>
	        ...
	        function buildCalendar() {
	            ...
	            days.forEach(e => {
	                e.addEventListener('click', ()=>{
	                    ...
	                    document.querySelector(".activity").value="종류"
	                    document.querySelector("#activitySet").setAttribute("hidden","")
	        		...
	        		document.querySelectorAll(".day").forEach(e => {
			    		...
			    		activeList.forEach(item=>{
			    			...
			    			if(item.activeDate!=null){
					    		day = item.activeDate.substring(6);
					    		console.log("12121212aaaa")
			    			}
			    			if(day==e.innerHTML.substring(0, 2)){
			    				daykcal-=Math.round(item.kcal)
			    				kcalFound=true
			    			}
			    		})
			    		if(kcalFound){
	...
	<body>
        <div>${dto.gender == 'man' ? '남자' : '여자'}, ${dto.height}cm, 체중 : ${dto.weight}kg, 표준체중 : <span id="aveWeight"></span>kg, 하루필요열량 : <span id="dayKcal"></span>kcal</div>  
		  ...
		  <div class="modal-dialog">
		    <div class="modal-content">
			  ...
			  <div class="modal-body">
				<h2 class="fs-5">운동량 등록하기</h2>
				<select class = "activity">
					<option>종류
					<option value="walk">걷기
					<option value="run">달리기
					<option value="cycle">자전거
				</select> 
				<div id="activitySet" hidden="">
					<div><input id="slope" placeholder="경사(평지라면 0입력)" type="number"><span>경사</span></div>
					<div><input id="speed" placeholder="속도(km/h)" type="number"><span>km/h</span></div>
					<div><input id="activeTime" placeholder="운동시간(분)" type="number"><span>분</span></div>
					<div><button type="button" id="activityBtn">등 록</button></div>			
				</div>
				
				<hr>
				<h2 class="fs-5">섭취 음식 등록하기</h2>
		        ...
		        <strong>운동량</strong>
		        <div>
		        	<small id="actSmall"></small>
		        </div>
		        <br>
		      	<div>일일 섭취 칼로리 : <span id="nowKcal">0</span>kcal</div>
		      	<div>일일 소모 칼로리 : <span id="nowActKcal">0</span>kcal</div>
		      	<div>일일 총 칼로리 : <span id="dayAllKcal">0</span>kcal</div>
		...
		if(document.querySelector(".eatType").value=='구분'){
		    ...
		    .then(response => {
		        if (!response.ok) {
			        ...
			        document.querySelector("#small").innerHTML+='<div>'+data.eatType+", 음식명 : "+data.foodName+", 칼로리 : "+data.kcal+"kcal</div>"
			        var actKcal =  parseInt(document.querySelector("#nowActKcal").innerHTML)
		            document.querySelector("#dayAllKcal").innerHTML=nowKcal-actKcal
					getDaysKcal()
		        }
		    })
    ...
    document.querySelector('#modalBtn').addEventListener('click', (e)=>{
        ...
        .then(data=>{
            ...
            const actSmall = document.querySelector('#actSmall');
            var dayAllKcal = document.querySelector('#dayAllKcal');
            small.innerHTML = ''; 
            actSmall.innerHTML='';
			var nowKcal=0
			var actKcal=0
            data.eatList.forEach(item => {
            	nowKcal+=item.kcal
                small.innerHTML += '<div>'+item.eatType+', 음식명 : '+item.foodName+',  칼로리 : '+item.kcal+'kcal</div>';
            });
            data.activeList.forEach(item => {
                ...
                actSmall.innerHTML += '<div>운동 : '+actType+', 시간 : '+item.time+'분,  칼로리 : '+item.kcal+'kcal</div>';
            });
            document.querySelector("#nowKcal").innerHTML=nowKcal
            document.querySelector("#nowActKcal").innerHTML=actKcal
            dayAllKcal.innerHTML=nowKcal-actKcal
        })
    })
	...
	activity.addEventListener('change', (e)=>{
	    ...
	    speed.value=""
	    activeTime.value=""
	    if(activity.value=="종류"){
	        document.querySelector("#activitySet").setAttribute("hidden","")
	    }else{
	        document.querySelector("#activitySet").removeAttribute("hidden")
	    }
	});

	activityBtn.addEventListener("click",()=>{
	    if(slope.value==""||speed.value==""||activeTime.value==""){
	        alert("모두 입력해주세요.")
	    }else{
	        ...
	        const data = {
	            ...
	            activeDate: document.querySelector("#exampleModalLabel").innerHTML
	        };
	        console.log(data)
	        fetch("/addActive", {
	            method: 'POST',
	        ...
	        .then(response=>{
	            if(response.ok){
	                alert("성공")
	                var nowActKcal = parseInt(document.querySelector("#nowActKcal").innerHTML);
	            	
	                activeKcal = 0
	                if(data.type=="cycle"){
    					activeKcal=(3.5+3.5+(1.8*data.speed/document.querySelector("#weight").value))*data.time
    				}else{
   						activeKcal=(data.speed*3.5*0.001*document.querySelector("#weight").value)*5*data.time
    				}
	                var actType = ""
	                if(data.type=="walk"){
	                	actType="걷기"
			        ...
			        nowActKcal += activeKcal; 
			        document.querySelector("#nowActKcal").innerHTML=nowActKcal;
			        document.querySelector("#actSmall").innerHTML+='<div>운동 : '+actType+", 시간 : "+data.time+"분, 칼로리 : "+activeKcal+"kcal</div>"
			        nowKcal = parseInt(document.querySelector("#nowKcal").innerHTML)
		            document.querySelector("#nowActKcal").innerHTML=nowActKcal
		            document.querySelector("#dayAllKcal").innerHTML=nowKcal-nowActKcal
					getDaysKcal()
	            }else{
	                alert("실패")
	            }
